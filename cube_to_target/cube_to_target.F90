...
!        STOP
    ...
    END IF
    !
    ! Additionally write a tile-based ridge topo file, in which the nrdg
    ! dimension is maxtiles (discrete ridge objects) rather than nsubr (angle
    ! bins). Written to a separate '_tiles.nc' file so the standard output
    ! above is untouched and the two can be compared side by side.
    !
    if (lridgetiles .and. lfind_ridges .and. .not.ltarget_latlon) then
      ofile = output_fname(1:LEN_TRIM(output_fname)-3)//'_tiles.nc'
      CALL wrtncdf_ridge_tiles(ntarget,ntiles_out,terr_target,landfrac_target,&
           sgh_target,sgh30_target,landm_coslat_target,target_center_lon,&
           target_center_lat,target_area,ofile,command_line_arguments,&
           str_creator,area_target,llandfrac)
    end if
