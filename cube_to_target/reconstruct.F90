...
!------------------------------------------------------------------------------
  SUBROUTINE MonotonizeABPGradient_New(fcubehalo, order, recons)
    ...
    REAL (KIND=dbl_kind) :: local_min, local_max, value, min_phi, linval
    REAL (KIND=dbl_kind) :: disc, mx, my
...
!------------------------------------------------------------------------------
  SUBROUTINE VanLeerLimit(fcubehalo, order, recons)
    ...
    DO i = 1, ncube_reconstruct-1
       ...
       min_phi = MIN(one, MAX(0.0_dbl_kind, (local_min - fcubehalo(i,j,k))/(recon_min - fcubehalo(i,j,k))), &
            MAX(0.0_dbl_kind, (local_max - fcubehalo(i,j,k))/(recon_max - fcubehalo(i,j,k))) )
...
!    USE CubedSphereTrans  ! Cubed sphere transforms
    ...
    ! Local variables
    INTEGER (KIND=int_kind) :: ii, iref, jj, imin, imax
    REAL    (KIND=dbl_kind) :: width, beta, a, newbeta
...
!    USE CubedSphereTrans  ! Cubed sphere transforms
    ...
    INTEGER (KIND=int_kind), INTENT(IN) :: nhalo
    INTEGER (KIND=int_kind), INTENT(IN) :: np, ncube
    ...
    REAL    (KIND=dbl_kind), DIMENSION(-nhalo:ncube+nhalo, nhalo+1) :: prealpha !changed compared to non-extended
    REAL    (KIND=dbl_kind), DIMENSION(-nhalo:ncube+nhalo, nhalo+1) :: newalpha !changed compared to non-extended
...
!    USE MathUtils         ! Has function for 1D cubic interpolation
    ...
    ! Local variables
    INTEGER (KIND=int_kind) :: ii, iref, ibaseref, jj, imin, imax
    REAL    (KIND=dbl_kind) :: width, beta, newbeta
