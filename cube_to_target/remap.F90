...
!    weights(:) = -half*(xseg(1)*yseg(2)-xseg(2)*yseg(1)) !dummy for testing
  ...
  subroutine get_weights_gauss(weights,xseg,yseg,nreconstruction,ngauss,gauss_weights,abscissae)
    ...
    !
    real (kind=real_kind) :: b,integral,dx2,xc,x,y
...
!    y = y_in
  ...
  REAL (KIND=dbl_kind) FUNCTION I_10_ab(alpha,beta)
    ...
    I_10_ab = -ASINH_local(COS(alpha) * TAN(beta))
  ...
  REAL (KIND=dbl_kind) FUNCTION I_01(x_in,y_in)
    ...
    y = y_in/aa
!    beta = ATAN(y)
!    I_01 = -ASINH_local(x*COS(beta))
    I_01 = -ASINH_local(x/SQRT(1+y*y))
  END FUNCTION I_01

  REAL (KIND=dbl_kind) FUNCTION I_01_ab(alpha,beta)
    IMPLICIT NONE
    REAL (KIND=dbl_kind), INTENT(IN) :: alpha,beta
    I_01_ab = -ASINH_local(COS(beta) * TAN(alpha))
...
!    I_20 = y*ASINH_local(COS(beta)*x)+ACOS(SIN(alpha)*SIN(beta))
    I_20 = y*ASINH_local(x/SQRT(tmp))+ACOS(x*y/(SQRT((one+x*x)*tmp)))
  END FUNCTION I_20
  ...
  REAL (KIND=dbl_kind) FUNCTION I_20_ab(alpha,beta)
    ...
    I_20_ab = TAN(beta)*ASINH_local(COS(beta)*TAN(alpha))+ACOS(SIN(alpha)*SIN(beta))
...
!    beta  = aa*ATAN(y)
    ...
    I_02 = x*ASINH_local(y/SQRT(tmp))+ACOS(x*y/SQRT(tmp*(1+y*y)))
  END FUNCTION I_02
  ...
  REAL (KIND=dbl_kind) FUNCTION I_02_ab(alpha,beta)
    ...
    I_02_ab = TAN(alpha)*ASINH_local(TAN(beta)*COS(alpha))+ACOS(SIN(alpha)*SIN(beta))
...
!------------------------------------------------------------------------------
! FUNCTION ASINH_local
!
! Description:
!   Hyperbolic arcsin function
!------------------------------------------------------------------------------
  FUNCTION ASINH_local(x)
    IMPLICIT NONE

    REAL (KIND=dbl_kind) :: ASINH_local
    REAL (KIND=dbl_kind) :: x

    ASINH_local = LOG(x + SQRT(x * x + one))
  END FUNCTION
