...
!=============================================================================
  SUBROUTINE smooth_intermediate_topo_wrap(terr, da, ncube,nhalo, NSCL_f,NSCL_c &
    ...
    REAL (KIND=dbl_kind), &
            DIMENSION(ncube,ncube,6)  :: rrfac1,terr_in
...
#ifdef IdToOLD
            write(*,*) "Overriding namelist setting to test code "
            write(*,*) "  ... only makes sense if smooth_fine=1 "
            use_prefilter=.FALSE.
#endif
            if (use_prefilter) then
            ! Need for consistency with CESM2 version. Also
            ! reduces number of searchable peaks which is good. 
               write(*,*) " pre - filter ***** with "
               write(*,*) " NSCL_f  =       ",NSCL_f
